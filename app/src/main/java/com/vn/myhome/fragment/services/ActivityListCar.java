...
public class ActivityListCar extends BaseActivity
    ...
    private void initData() {
        if (App.mListCar != null && App.mListCar.size() > 0) {
            mList.addAll(App.mListCar);
            temp.addAll(mList);
            adapterService.notifyDataSetChanged();
        } else {
            showDialogLoading();
            mPresenter.api_get_type_car(SharedPrefs.getInstance().get(Constants.KEY_SAVE_USERNAME, String.class));
        }
    }
